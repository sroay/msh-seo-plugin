...
Tags: seo, schema, xml sitemap, redirects, meta description
...
1. The MSH SEO panel in the block editor: focus keyword and an SEO score from 14 on-page checks
2. Meta title and description with a live search result preview and character counts
3. Social preview for Facebook, LinkedIn and X, and the per-post schema type selector
4. The SEO overview: site score, issues sorted by impact, and one-click fixes
5. The redirect manager with the 404 log underneath
6. Settings: feature status, indexing, and the optional Marketing So High connection
